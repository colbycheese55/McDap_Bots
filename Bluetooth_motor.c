...
// BLE Joystick control -> Drive motors
// Mapping (uppercase only; lowercase ignored):
//   A = forward, C = backward, B = turn right in place, D = turn left in place
//   '0' = release/stop
//
...
#define DRIVE_SPEED  0.60f   // tweak as needed [0..1]

typedef enum {
    DRIVE_IDLE = 0,
    DRIVE_FWD,
    DRIVE_REV,
    DRIVE_TURN_LEFT_IN_PLACE,
    DRIVE_TURN_RIGHT_IN_PLACE
} drive_mode_t;

static drive_mode_t g_drive_mode = DRIVE_IDLE;

static void drive_apply(drive_mode_t m){
    g_drive_mode = m;
    switch(m){
        default:
        case DRIVE_IDLE:
        ...
        case DRIVE_FWD:
            motor_set_speed_left(+DRIVE_SPEED);
            motor_set_speed_right(+DRIVE_SPEED);
            LED_on();
            break;
        ...
        case DRIVE_TURN_LEFT_IN_PLACE:
            motor_set_speed_left(-DRIVE_SPEED);
            motor_set_speed_right(+DRIVE_SPEED);
            LED_on();
            break;

        case DRIVE_TURN_RIGHT_IN_PLACE:
            motor_set_speed_left(+DRIVE_SPEED);
            motor_set_speed_right(-DRIVE_SPEED);
            LED_on();
            break;
...
/* Interpret joystick bytes:
   Uppercase only: A,B,C,D control; '0' means release/stop.
   Lowercase (repeat/short press noise) is ignored by design. */
static void handle_bytes(const uint8_t *data, uint16_t n){
    for(uint16_t i=0;i<n;i++){
        char ch = (char)data[i];

        if (ch == 0x00){
            drive_apply(DRIVE_IDLE);
            DBG_write("DRIVE: stop (release '0')\r\n");
            continue;
        }

        switch(ch){
            case 'A':  // forward
                drive_apply(DRIVE_FWD);
                DBG_write("DRIVE: forward ('A')\r\n");
                break;
            case 'C':  // backward
                drive_apply(DRIVE_REV);
                DBG_write("DRIVE: reverse ('C')\r\n");
                break;
            case 'B':  // turn right in place
                drive_apply(DRIVE_TURN_RIGHT_IN_PLACE);
                DBG_write("DRIVE: turn right in place ('B')\r\n");
                break;
            case 'D':  // turn left in place
                drive_apply(DRIVE_TURN_LEFT_IN_PLACE);
                DBG_write("DRIVE: turn left in place ('D')\r\n");
                break;
            default:
...
int main(void){
    ...
    // If you already initialize motors elsewhere, you can remove these lines.
    // Motor_Handle mh = {
