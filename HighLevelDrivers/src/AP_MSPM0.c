...
// Updated to new SysConfig names:
//   MASTER_RDY  -> BLUETOOTH_GPIO_MASTER_RDY_*
//   SLAVE_RDY   -> BLUETOOTH_GPIO_SLAVE_RDY_*
//   RESET       -> BLUETOOTH_GPIO_RESET_*
//
// MRDY: host -> SNP  (assert = 0, deassert = 1)
// SRDY: SNP  -> host (assert = 0, deassert = 1)  [read-only]
// RESET: host -> SNP (assert reset = 0, release = 1)

void SetMRDY(void)   { DL_GPIO_setPins  (BLUETOOTH_GPIO_MASTER_RDY_PORT, BLUETOOTH_GPIO_MASTER_RDY_PIN); } // 1 = not ready
void ClearMRDY(void) { DL_GPIO_clearPins(BLUETOOTH_GPIO_MASTER_RDY_PORT, BLUETOOTH_GPIO_MASTER_RDY_PIN); } // 0 = ready

void SetReset(void)   { DL_GPIO_setPins  (BLUETOOTH_GPIO_RESET_PORT, BLUETOOTH_GPIO_RESET_PIN); }          // 1 = run
void ClearReset(void) { DL_GPIO_clearPins(BLUETOOTH_GPIO_RESET_PORT, BLUETOOTH_GPIO_RESET_PIN); }          // 0 = reset

uint32_t ReadSRDY(void){
    return (DL_GPIO_readPins(BLUETOOTH_GPIO_SLAVE_RDY_PORT, BLUETOOTH_GPIO_SLAVE_RDY_PIN) ? 1u : 0u);
}
