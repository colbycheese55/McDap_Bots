...
//
// New names from ti_msp_dl_config.h:
//   BLUETOOTH_GPIO_MASTER_RDY_PORT / PIN  (MRDY)
//   BLUETOOTH_GPIO_SLAVE_RDY_PORT  / PIN  (SRDY)
//   BLUETOOTH_GPIO_RESET_PORT      / PIN  (RESET)

void SetMRDY(void)
{
    // 1 = not ready (deassert)
    DL_GPIO_setPins(BLUETOOTH_GPIO_MASTER_RDY_PORT,
                    BLUETOOTH_GPIO_MASTER_RDY_PIN);
}

void ClearMRDY(void)
{
    // 0 = ready (assert)
    DL_GPIO_clearPins(BLUETOOTH_GPIO_MASTER_RDY_PORT,
                      BLUETOOTH_GPIO_MASTER_RDY_PIN);
}
...
{
    // 1 = run (release reset)
    DL_GPIO_setPins(BLUETOOTH_GPIO_RESET_PORT,
                    BLUETOOTH_GPIO_RESET_PIN);
}

void ClearReset(void)
{
    // 0 = reset (hold in reset)
    DL_GPIO_clearPins(BLUETOOTH_GPIO_RESET_PORT,
                      BLUETOOTH_GPIO_RESET_PIN);
}

uint32_t ReadSRDY(void)
{
    // SRDY asserted low by SNP when it has data / is ready
    return (DL_GPIO_readPins(BLUETOOTH_GPIO_SLAVE_RDY_PORT,
                             BLUETOOTH_GPIO_SLAVE_RDY_PIN)
            ? 1u : 0u);
}
